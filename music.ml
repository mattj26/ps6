...
(* Alternate implementatin of stream_to_hex that is recursive *)
(* 
let rec stream_to_hex (n : int) (str : event stream) =
  if n = 0 then "" else
  match str () with
    | Cons(Tone (t, pitch, vol), tl) -> 
        (time_to_hex t)^"90"^(pitch_to_hex pitch)^(int_to_hex vol)^
    ...
    | Cons(Stop (t, pitch), tl) ->
        (time_to_hex t)^(pitch_to_hex pitch)^"00"^(stream_to_hex (n-1)
         tl)
*)

let output_midi filename hex =
...
*)
(*******
COMMENT BY Stuart: Add an invariant that the volume of a note
cannot be 0. This prevents "ghost tones" in list_to_stream.
********)
...
(* 
......................................................................
Then with just three lists provided after this comment and 
